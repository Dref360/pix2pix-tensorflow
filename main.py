...
parser.add_argument('--mean', dest='mean', type=int, default=64, help='# images used to train')
parser.add_argument('--std', dest='std', type=int, default=32, help='# images used to train')
    ...
    with tf.device('/cpu:0'):
        with tf.Session(config=tf.ConfigProto(**cfg)) as sess:
            model = pix2pix(sess, image_size=args.fine_size, batch_size=args.batch_size,
                            ...
                            checkpoint_dir=args.checkpoint_dir, sample_dir=args.sample_dir,mean=args.mean,std=args.std)
